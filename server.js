...
app.post('/gerar-pdf', (req, res) => {
  ...
  doc.fontSize(14).font('Helvetica').text('oportunidade que foi autuado pelo suposto cometimento da infração prevista no Art. 170 do Código de Trânsito Brasileiro.', { align: 'left' });
  doc.moveDown();
  ...
  doc.fontSize(14).font('Helvetica').text('Em complemento ao anteriormente tratado, é importante dizer ainda que além de observância obrigatória do Código de Trânsito Brasileiro, deve os órgãos julgadores de trânsito subordinação cogente ao cumprimento do Manual Brasileiro de Fiscalização de Trânsito, posto que é essencial para a atuação de primeira e segunda instância, haja vista que contém a interpretação correta acerca da aplicação da legislação de trânsito pelos órgãos competentes, ou seja, pelo SENATRAN e CONTRAN.',
  { align: 'left' });
  doc.moveDown();
...
doc.moveDown();
  ...
  doc.fontSize(14).font('Helvetica').text('A titulo de exemplo verifica-se que o art. 170 do CTB, com base na observância do MBFT, prevê no campo “Quando Autuar” a possibilidade de imputação de penalidade em várias possibilidades, ou seja, condutar distintas.', { align: 'left' });
  doc.moveDown();
...
*/
//-------------------------------------------------------------------------------------------
  const pdfFilename = 'Petição Modelo Art.170.pdf';
